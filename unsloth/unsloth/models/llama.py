...
def LlamaAttention_fast_forward_inference(
    ...
    if do_prefill:
        self.paged_attention = torch.empty((KV_CACHE_INCREMENT + seq_len + 1, 2, bsz, n_kv_heads, head_dim),
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        self.register_buffer("cos_cached", emb.cos().to(dtype=dtype, device=device, non_blocking=True),
                             persistent=False)
        self.register_buffer("sin_cached", emb.sin().to(dtype=dtype, device=device, non_blocking=True),
                             persistent=False)
